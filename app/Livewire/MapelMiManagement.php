...
use App\Exports\MapelMiExport;
use App\Exports\MapelMiTemplateExport;
use App\Imports\MapelMiImport;
...
class MapelMiManagement extends Component
{
    ...
    {
        return [
            'kode_mapel' => ['nullable', 'string', 'max:20', Rule::unique('mapel_mis', 'kode_mapel')->ignore($this->mapelId)],
    ...
    public function sortItem($itemId, $position): void
    {
        ...
        $items = MapelMi::orderBy('sort_order')->orderBy('id')->pluck('id')->toArray();
        ...
        foreach ($items as $index => $id) {
            MapelMi::where('id', $id)->update(['sort_order' => $index]);
        }
    }
    ...
    public function save(): void
    {
        ...
        if ($this->isEditing) {
            $mapel = MapelMi::findOrFail($this->mapelId);
            $mapel->update($validated);
            session()->flash('success', 'Data mata pelajaran MI berhasil diperbarui.');
        } else {
            $maxOrder = MapelMi::max('sort_order') ?? 0;
            $validated['sort_order'] = $maxOrder + 1;
            MapelMi::create($validated);
            session()->flash('success', 'Data mata pelajaran MI berhasil ditambahkan.');
        }
    ...
    public function delete(): void
    {
        ...
        $this->mapelId = null;
        session()->flash('success', 'Data mata pelajaran MI berhasil dihapus.');
    }
    ...
    public function deleteAll(): void
    {
        ...
        session()->flash('success', "Semua data mata pelajaran MI ($count data) berhasil dihapus.");
    }
    ...
    public function import(): void
    {
        ...
        try {
            ...
            if ($import->getErrors()) {
                ...
                session()->flash('success', 'Data mata pelajaran MI berhasil diimport. Total: ' . $import->getRowCount() . ' data.');
    ...
    {
        return Excel::download(new MapelMiExport(), 'data-mapel-mi-' . date('Y-m-d-His') . '.xlsx');
    }

    public function downloadTemplate()
    {
        return Excel::download(new MapelMiTemplateExport(), 'template-import-mapel-mi.xlsx');
    }
    ...
    public function render()
    {
        ...
        return view('livewire.mapel-mi-management', [
            'mapels' => $mapels,
        ])->layout('layouts.admin', ['header' => 'Data Mata Pelajaran MI']);
    }
}
