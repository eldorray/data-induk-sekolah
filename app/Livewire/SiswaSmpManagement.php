...
use App\Exports\SiswaSmpExport;
use App\Exports\SiswaSmpTemplateExport;
...
class SiswaSmpManagement extends Component
{
    ...
    protected function rules(): array
    {
        return [
            ...
            'nisn' => ['nullable', 'string', 'max:20', Rule::unique('siswa_smps', 'nisn')->ignore($this->siswaId)],
            'nik' => ['nullable', 'string', 'max:20', Rule::unique('siswa_smps', 'nik')->ignore($this->siswaId)],
    ...
    public function save(): void
    {
        ...
        if ($this->isEditing) {
            $siswa = SiswaSmp::findOrFail($this->siswaId);
            $siswa->update($validated);
            session()->flash('success', 'Data siswa SMP berhasil diperbarui.');
        } else {
            SiswaSmp::create($validated);
            session()->flash('success', 'Data siswa SMP berhasil ditambahkan.');
        }
    ...
    public function delete(): void
    {
        ...
        $this->siswaId = null;
        session()->flash('success', 'Data siswa SMP berhasil dihapus.');
    }
    ...
    public function deleteAll(): void
    {
        ...
        SiswaSmp::query()->delete();
        $this->showDeleteAllModal = false;
        session()->flash('success', "Semua data siswa SMP ($count data) berhasil dihapus.");
    }
    ...
    public function import(): void
    {
        ...
        try {
            ...
            if ($import->getErrors()) {
                ...
                session()->flash('success', 'Data siswa SMP berhasil diimport. Total: '.$import->getRowCount().' data.');
    ...
    {
        return Excel::download(new SiswaSmpExport, 'data-siswa-smp-'.date('Y-m-d-His').'.xlsx');
    }

    public function downloadTemplate()
    {
        return Excel::download(new SiswaSmpTemplateExport, 'template-import-siswa-smp.xlsx');
    }

    public function render()
    {
        $siswas = SiswaSmp::query()
        ...
        return view('livewire.siswa-smp-management', [
            'siswas' => $siswas,
        ])->layout('layouts.admin', ['header' => 'Data Siswa SMP']);
    }
}
