...
use App\Exports\GuruSmpExport;
use App\Exports\GuruSmpTemplateExport;
use App\Imports\GuruSmpImport;
...
class GuruSmpManagement extends Component
{
    ...
    protected function rules(): array
    {
        $uniqueRule = $this->isEditing 
            ? 'required|string|size:16|unique:guru_smps,nik,' . $this->guruId
            : 'required|string|size:16|unique:guru_smps,nik';
    ...
    public function save(): void
    {
        ...
        if ($this->sk_awal) {
            ...
            $data['sk_awal_path'] = $this->sk_awal->store('guru-smp/sk', 'public');
        }
        ...
        if ($this->sk_akhir) {
            ...
            $data['sk_akhir_path'] = $this->sk_akhir->store('guru-smp/sk', 'public');
        }

        if ($this->isEditing) {
            $guru = GuruSmp::findOrFail($this->guruId);
            $guru->update($data);
            session()->flash('success', 'Data guru SMP berhasil diperbarui.');
        } else {
            GuruSmp::create($data);
            session()->flash('success', 'Data guru SMP berhasil ditambahkan.');
        }
    ...
    public function delete(): void
    {
        ...
        $this->guruId = null;
        session()->flash('success', 'Data guru SMP berhasil dihapus.');
    }
    ...
    {
        // Delete all SK files
        $gurus = GuruSmp::whereNotNull('sk_awal_path')->orWhereNotNull('sk_akhir_path')->get();
        ...
        session()->flash('success', 'Semua data guru SMP berhasil dihapus.');
    }
    ...
    {
        return Excel::download(new GuruSmpExport, 'data-guru-smp-' . date('Y-m-d') . '.xlsx');
    }

    // Download Template
    public function downloadTemplate()
    {
        return Excel::download(new GuruSmpTemplateExport, 'template-import-guru-smp.xlsx');
    }
    ...
    public function import(): void
    {
        ...
        try {
            ...
            if (count($errors) > 0) {
                ...
                session()->flash('success', 'Data guru SMP berhasil diimport.');
            }
        } catch (\Exception $e) {
    ...
    public function render()
    {
        ...
        return view('livewire.guru-smp-management', [
            'gurus' => $gurus,
        ])->layout('layouts.admin', ['header' => 'Data Guru SMP']);
    }
}
