...
use App\Exports\SiswaMiExport;
use App\Exports\SiswaMiTemplateExport;
use App\Imports\SiswaMiImport;
...
class SiswaMiManagement extends Component
{
    ...
    protected function rules(): array
    {
        return [
            ...
            'nisn' => ['nullable', 'string', 'max:20', Rule::unique('siswa_mis', 'nisn')->ignore($this->siswaId)],
            'nik' => ['nullable', 'string', 'max:20', Rule::unique('siswa_mis', 'nik')->ignore($this->siswaId)],
    ...
    public function save(): void
    {
        ...
        if ($this->isEditing) {
            $siswa = SiswaMi::findOrFail($this->siswaId);
            $siswa->update($validated);
            session()->flash('success', 'Data siswa MI berhasil diperbarui.');
        } else {
            SiswaMi::create($validated);
            session()->flash('success', 'Data siswa MI berhasil ditambahkan.');
        }
    ...
    public function delete(): void
    {
        ...
        $this->siswaId = null;
        session()->flash('success', 'Data siswa MI berhasil dihapus.');
    }
    ...
    public function deleteAll(): void
    {
        ...
        SiswaMi::query()->delete();
        $this->showDeleteAllModal = false;
        session()->flash('success', "Semua data siswa MI ($count data) berhasil dihapus.");
    }
    ...
    public function import(): void
    {
        ...
        try {
            ...
            if ($import->getErrors()) {
                ...
                session()->flash('success', 'Data siswa MI berhasil diimport. Total: '.$import->getRowCount().' data.');
    ...
    {
        return Excel::download(new SiswaMiExport, 'data-siswa-mi-'.date('Y-m-d-His').'.xlsx');
    }

    public function downloadTemplate()
    {
        return Excel::download(new SiswaMiTemplateExport, 'template-import-siswa-mi.xlsx');
    }

    public function render()
    {
        $siswas = SiswaMi::query()
        ...
        return view('livewire.siswa-mi-management', [
            'siswas' => $siswas,
        ])->layout('layouts.admin', ['header' => 'Data Siswa MI']);
    }
}
