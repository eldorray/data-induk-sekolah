...
use App\Exports\GuruMiExport;
use App\Exports\GuruMiTemplateExport;
use App\Imports\GuruMiImport;
...
class GuruMiManagement extends Component
{
    ...
    protected function rules(): array
    {
        $uniqueRule = $this->isEditing 
            ? 'required|string|size:16|unique:guru_mis,nik,' . $this->guruId
            : 'required|string|size:16|unique:guru_mis,nik';
    ...
    public function save(): void
    {
        ...
        if ($this->sk_awal) {
            ...
            $data['sk_awal_path'] = $this->sk_awal->store('guru-mi/sk', 'public');
        }
        ...
        if ($this->sk_akhir) {
            ...
            $data['sk_akhir_path'] = $this->sk_akhir->store('guru-mi/sk', 'public');
        }

        if ($this->isEditing) {
            $guru = GuruMi::findOrFail($this->guruId);
            $guru->update($data);
            session()->flash('success', 'Data guru MI berhasil diperbarui.');
        } else {
            GuruMi::create($data);
            session()->flash('success', 'Data guru MI berhasil ditambahkan.');
        }
    ...
    public function delete(): void
    {
        ...
        $this->guruId = null;
        session()->flash('success', 'Data guru MI berhasil dihapus.');
    }
    ...
    {
        // Delete all SK files
        $gurus = GuruMi::whereNotNull('sk_awal_path')->orWhereNotNull('sk_akhir_path')->get();
        ...
        session()->flash('success', 'Semua data guru MI berhasil dihapus.');
    }
    ...
    {
        return Excel::download(new GuruMiExport, 'data-guru-mi-' . date('Y-m-d') . '.xlsx');
    }

    // Download Template
    public function downloadTemplate()
    {
        return Excel::download(new GuruMiTemplateExport, 'template-import-guru-mi.xlsx');
    }
    ...
    public function import(): void
    {
        ...
        try {
            ...
            if (count($errors) > 0) {
                ...
                session()->flash('success', 'Data guru MI berhasil diimport.');
            }
        } catch (\Exception $e) {
    ...
    public function render()
    {
        ...
        return view('livewire.guru-mi-management', [
            'gurus' => $gurus,
        ])->layout('layouts.admin', ['header' => 'Data Guru MI']);
    }
}
