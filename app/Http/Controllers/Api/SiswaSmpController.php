...
class SiswaSmpController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Data siswa SMP berhasil diambil',
            'data' => $siswas->items(),
            'meta' => [
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            ...
            'nisn' => 'nullable|string|max:20|unique:siswa_smps,nisn',
            'nik' => 'nullable|string|max:20|unique:siswa_smps,nik',
        ...
        $siswa = SiswaSmp::create($validated);

        return response()->json([
            'success' => true,
            'message' => 'Data siswa SMP berhasil ditambahkan',
            'data' => $siswa,
        ], 201);
    }
     ...
     */
    public function show(SiswaSmp $siswaSmp): JsonResponse
    {
        return response()->json([
            'success' => true,
            'message' => 'Data siswa SMP berhasil diambil',
     ...
     */
    public function update(Request $request, SiswaSmp $siswaSmp): JsonResponse
    {
        $validated = $request->validate([
            'nama_lengkap' => 'required|string|max:255',
            'nisn' => ['nullable', 'string', 'max:20', Rule::unique('siswa_smps', 'nisn')->ignore($siswaSmp->id)],
            'nik' => ['nullable', 'string', 'max:20', Rule::unique('siswa_smps', 'nik')->ignore($siswaSmp->id)],
            'tempat_lahir' => 'nullable|string|max:100',
            'tanggal_lahir' => 'nullable|date',
            'tingkat_rombel' => 'nullable|string|max:100',
            'status' => 'nullable|string|in:Aktif,Tidak Aktif,Lulus,Pindah,Keluar',
        ...
        $siswaSmp->update($validated);

        return response()->json([
            'success' => true,
            'message' => 'Data siswa SMP berhasil diperbarui',
            'data' => $siswaSmp,
        ]);
    }
     ...
     */
    public function destroy(SiswaSmp $siswaSmp): JsonResponse
    {
        $siswaSmp->delete();

        return response()->json([
            'success' => true,
            'message' => 'Data siswa SMP berhasil dihapus',
        ]);
    }
    ...
    public function sync(Request $request): JsonResponse
    {
        ...
        foreach ($validated['data'] as $index => $row) {
            try {
                ...
                $siswa = null;
                if (!empty($row['nisn'])) {
                    $siswa = SiswaSmp::where('nisn', $row['nisn'])->first();
                }
                if (!$siswa && !empty($row['nik'])) {
                    $siswa = SiswaSmp::where('nik', $row['nik'])->first();
                }
