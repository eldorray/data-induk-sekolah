...
class GuruMiController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Data guru MI berhasil diambil',
            'data' => $gurus->items(),
            'meta' => [
    ...
    public function store(Request $request): JsonResponse
    {
        $validated = $request->validate([
            'nip' => 'nullable|string|max:30|unique:guru_mis,nip',
            'nuptk' => 'nullable|string|max:30|unique:guru_mis,nuptk',
            'npk' => 'nullable|string|max:30',
            'nik' => 'nullable|string|max:20|unique:guru_mis,nik',
        ...
        $guru = GuruMi::create($validated);

        return response()->json([
            'success' => true,
            'message' => 'Data guru MI berhasil ditambahkan',
            'data' => $guru,
        ], 201);
    }
     ...
     */
    public function show(GuruMi $guruMi): JsonResponse
    {
        return response()->json([
            'success' => true,
            'message' => 'Data guru MI berhasil diambil',
     ...
     */
    public function update(Request $request, GuruMi $guruMi): JsonResponse
    {
        $validated = $request->validate([
            'nip' => ['nullable', 'string', 'max:30', Rule::unique('guru_mis', 'nip')->ignore($guruMi->id)],
            'nuptk' => ['nullable', 'string', 'max:30', Rule::unique('guru_mis', 'nuptk')->ignore($guruMi->id)],
            'npk' => 'nullable|string|max:30',
            'nik' => ['nullable', 'string', 'max:20', Rule::unique('guru_mis', 'nik')->ignore($guruMi->id)],
            'front_title' => 'nullable|string|max:50',
            'full_name' => 'required|string|max:255',
            'back_title' => 'nullable|string|max:50',
            'gender' => 'nullable|in:L,P',
        ...
        $guruMi->update($validated);

        return response()->json([
            'success' => true,
            'message' => 'Data guru MI berhasil diperbarui',
            'data' => $guruMi,
        ]);
    }
     ...
     */
    public function destroy(GuruMi $guruMi): JsonResponse
    {
        $guruMi->delete();

        return response()->json([
            'success' => true,
            'message' => 'Data guru MI berhasil dihapus',
        ]);
    }
    ...
    public function sync(Request $request): JsonResponse
    {
        ...
        foreach ($validated['data'] as $index => $row) {
            try {
                ...
                $guru = null;
                if (!empty($row['nip'])) {
                    $guru = GuruMi::where('nip', $row['nip'])->first();
                }
                if (!$guru && !empty($row['nuptk'])) {
                    $guru = GuruMi::where('nuptk', $row['nuptk'])->first();
                }
                if (!$guru && !empty($row['nik'])) {
                    $guru = GuruMi::where('nik', $row['nik'])->first();
                }
